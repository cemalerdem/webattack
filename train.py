...
def train(sample_pred_text):
    max_features = 26773
    db = pandas.read_csv(csv_file, engine='python', quotechar='|', header=None)
    db.isnull().values.any()
    dataset = db.sample(frac=1).values
    dataset.shape
    ...
    # Save model
    model.save_weights('saved_model/securitai-lstm-weights.h5')
    model.save('saved_model/securitai-lstm-model.h5')
    with open('saved_model/securitai-lstm-model.json', 'w') as outfile:
